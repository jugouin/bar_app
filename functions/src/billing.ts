...
import { sendClientEmail, sendAdminEmail } from "./emails";
...
export async function runMonthlyBilling(): Promise<void> {
  ...
  console.log(`[monthlyBilling] Période: ${start.toISOString()} → ${end.toISOString()}`);
  ...
  for (const user of Object.values(byUser)) {
    ...
    console.log(`[Checkout] Création pour ${user.email} — ${totalCents} centimes`);
  ...
  console.log(`[monthlyBilling] Terminé — ${nbOk}/${checkoutResults.length} OK`);
}